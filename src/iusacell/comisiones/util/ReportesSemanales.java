...
public class ReportesSemanales {
	...
	//public static String ACTIVACIONES = 
	  /*"SELECT     pa.cdg_region 							REGION,					" +
		...
		"AND PA.fec_alta_cliente >= TO_DATE(? ||' 000000','YYYYMMDD HH24MISS')	" + // parametro '20160201'
		"AND PA.fec_alta_cliente <= TO_DATE(? ||' 235959','YYYYMMDD HH24MISS')	" + // parametro '20160207'
		"ORDER BY 1 DESC"; */
	 
	public static String ACTIVACIONES =  "SELECT  pa.cdg_region  REGION, "+                     
    "pc.pc_Cve_paquete                        CVE_PAQUETE, "+             
    "pc_desc_larga_plan                        DESC_LARGA_PLAN,  "+         
    "PC.PC_CVE_CANAL                            CVE_CANAL,   "+               
    "CV.pc_DESC_CANAL                        DESC_CANAL,     "+             
...
"ORDER BY 1 DESC" ;
	...
	/*public static String RENOVACIONES = 
		...
		"       AND D.PC_CVE_DIVISION (+)  = SI.PC_CVE_DIVISION	" +
		"       AND PC.PC_CVE_CANAL = CV.PC_CVE_CANAL			" +
		"       AND Pc.pc_fecha_renova >= TO_DATE(? ||' 000000','YYYYMMDD HH24MISS') " + // PARAMETRO '20160201'
		"       AND pc.pc_fecha_renova <= TO_DATE(? ||' 235959','YYYYMMDD HH24MISS') " + // PARAMETRO '20160207'
		" 	ORDER BY 1 DESC"; */
	...
	/*public static String COBRANZA = 
		...
		"	     prodcm.SIRCESTADOS se,				" +
		"	     prodcm.PC_DIVISIONES d             " +        
		"	WHERE pc_fec_trans >= TO_DATE(? ||' 000000','YYYYMMDD HH24MISS')		" + //PARAMETROS '20160201'
		"		AND   pc_fec_trans <= TO_DATE(? ||' 235959','YYYYMMDD HH24MISS')	" + //PARAMETROS '20160207'
		...
		"		AND  se.edoclclave(+) = si.edoclclave					" +
		"		AND  d.pc_cve_division(+) = si.pc_cve_division 			" +
		"	GROUP BY c.pc_cve_canal, CV.pc_desc_canal, pc_desc_cpto, c.cdg_region, c.cdg_csi, si.pobkpnombre, pc_des_division, se.edodenombre	" + 
